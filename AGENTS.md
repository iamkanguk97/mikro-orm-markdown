...
"Schema declaration" means the exported `new EntitySchema({...})` / `defineEntity({...})` variable. For a class-linked schema (`class:`), property JSDoc comes from the class unconditionally; entity-level JSDoc merges field by field with the class winning conflicts, except `@hidden`, which applies when either location has it. For a name-only schema (no `class:`), property JSDoc is read from inside the `properties` object literal (v7's `properties: (p) => ({...})` builder callback included); in the rendered table it beats the `comment` property option, which stays the fallback.
