...
- The public API — `generateMarkdown()`, `resolveJsDocSources()`, the `mikro-orm-markdown` CLI and its options, and the supported JSDoc tags (`@namespace`, `@erd`, `@describe`, `@hidden`, `@atLeastOne`) — is now stable and covered by semantic versioning: breaking changes will only ship in future major versions
